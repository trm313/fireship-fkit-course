<script lang="ts">
  ...
  // custom event dispatcher that can be listened to from the parent component
  ...
  // When the sorting is done, the dispatch lets the parent event be aware of it
...
{#if list?.length}
  <ul class="list-none p-0 flex flex-col items-center">
    <!-- extracting index, and assigning `item.id` as the key -->
    {#each list as item, index (item.id)}
      <li
        class="w-full max-w-md border-2 border-dashed border-transparent p-2"
...
{:else}
  <p class="text-center my-12 text-lg font-bold">
